...
(function () {
    ...
    const apiUrl = 'https://openrouter-proxy.1067531410.workers.dev';  // 这是 Worker 的 URL
    ...
    async function sendMsg() {
        ...
        try {
            if (!apiKey) {
                throw new Error('API 密钥未设置，请检查环境变量');
            }
            console.log('使用的 API 密钥:', apiKey); // 打印使用的 API 密钥，用于调试
            const headers = {
                'Authorization': `Bearer ${apiKey}`,
                'Content-Type': 'application/json'
            };
            console.log('请求头:', headers); // 打印请求头，用于调试
            ...
            console.log('API 响应状态:', res.status); // 打印 API 响应状态，用于调试
            if (!res.ok) {
                if (res.status === 401) {
                    const errorBody = await res.text();
                    console.log('401 错误响应内容:', errorBody); // 打印 401 错误响应内容，用于调试
                    throw new Error('身份验证失败，请检查 API 密钥');
                }
